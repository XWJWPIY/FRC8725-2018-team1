...
public class ElevatorTurningModule {
    ...
    public ElevatorTurningModule(int MotorId, boolean MotorReversed) {
        Motor = new CANSparkMax(MotorId, MotorType.kBrushless);
        this.MotorReversed = MotorReversed;

        Motor.setSmartCurrentLimit(30);

        Motor.setIdleMode(IdleMode.kBrake); // 設為無動力時以鎖定

        Motor.setInverted(MotorReversed); // 是否反轉

        Encoder = Motor.getEncoder(); // 取得現在編碼器的值

        Encoder.setPositionConversionFactor(ElevatorModuleConstants.kElevatorLREncoderRot2Meter); // 轉換單位 to (m)
    }
    ...
    public void Running(boolean Running, boolean reverse) { 
        if (Running == true) { // 若是啟動
            if (reverse) { // 方向是否反轉
                Motor.set(ElevatorModuleConstants.kTurningSpeed); // 逆
            } else {
                Motor.set(-ElevatorModuleConstants.kTurningSpeed); // 順
            }
        }
        getDrivePosition();
        putDashboard(); // 功能在底下
